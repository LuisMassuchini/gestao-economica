                    ...
                    <form action="{{ route('units.update', $unit) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="trading_name" id="trading_name" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600" value="{{ old('trading_name', $unit->trading_name) }}" required>
                            </div>

                            <!-- Campo Razão Social -->
                            <div>
                                <label for="company_name" class="block font-medium text-sm text-gray-700 dark:text-gray-300">Razão Social</label>
                                <input type="text" name="company_name" id="company_name" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600" value="{{ old('company_name', $unit->company_name) }}" required>
                            </div>

                            <!-- Campo CNPJ -->
                            <div>
                                <label for="cnpj" class="block font-medium text-sm text-gray-700 dark:text-gray-300">CNPJ</label>
                                <input type="text" name="cnpj" id="cnpj" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600" value="{{ old('cnpj', $unit->cnpj) }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="flag_id" id="flag_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600" required>
                                    ...
                                    @foreach($flags as $flag)
                                        <option value="{{ $flag->id }}" {{ old('flag_id', $unit->flag_id) == $flag->id ? 'selected' : '' }}>
                                            {{ $flag->name }}
                                        </option>
                                    @endforeach
                                </select>
