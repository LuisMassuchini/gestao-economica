                    ...
                    <form action="{{ route('collaborators.update', $collaborator) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="name" id="name" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600" value="{{ old('name', $collaborator->name) }}" required>
                            </div>

                            <!-- Campo E-mail -->
                            <div>
                                <label for="email" class="block font-medium text-sm text-gray-700 dark:text-gray-300">E-mail</label>
                                <input type="email" name="email" id="email" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600" value="{{ old('email', $collaborator->email) }}" required>
                            </div>

                            <!-- Campo CPF -->
                            <div>
                                <label for="cpf" class="block font-medium text-sm text-gray-700 dark:text-gray-300">CPF</label>
                                <input type="text" name="cpf" id="cpf" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600" value="{{ old('cpf', $collaborator->cpf) }}" required>
                            </div>
                            ...
                            <div>
                                ...
                                <select name="unit_id" id="unit_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300 dark:bg-gray-700 dark:border-gray-600" required>
                                    ...
                                    @foreach($units as $unit)
                                        <option value="{{ $unit->id }}" {{ old('unit_id', $collaborator->unit_id) == $unit->id ? 'selected' : '' }}>
                                            {{ $unit->trading_name }}
                                        </option>
                                    @endforeach
                                </select>
